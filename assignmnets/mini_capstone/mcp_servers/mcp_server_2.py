...
if __name__ == "__main__":
    # When called via stdio (by MultiMCP), just run the server directly
    # When called directly with "dev", run without transport
    # When called directly without args, run with stdio and process docs
    
    if len(sys.argv) > 1 and sys.argv[1] == "dev":
        mcp.run() # Run without transport for dev server
    elif len(sys.argv) > 1 and sys.argv[1] == "stdio":
        # Called by MultiMCP via stdio - run directly (this is the normal case)
        mcp.run(transport="stdio")
    else:
        # Called directly without args - run with stdio in background and process docs
        import threading
